...
df = pd.DataFrame()
for i in np.random.randint(0, 398, 5):
    if df.empty:
...
sub_test_df = resample(general_test_df, n_samples=int(general_test_df.shape[0]*0.01))
...
# result_df.to_csv("submit_log5.csv", index=False)
# %%
